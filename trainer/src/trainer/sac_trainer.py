...
class SACTrainer:
    def __init__(self, env, config, save_dir="results/sac"):
        ...
        self.critic1 = QNetwork(env.observation_space.shape[0], action_dim, config.get("hidden_dim", 256)).to(self.device)
        self.critic2 = QNetwork(env.observation_space.shape[0], action_dim, config.get("hidden_dim", 256)).to(self.device)
        self.target_critic1 = QNetwork(env.observation_space.shape[0], action_dim, config.get("hidden_dim", 256)).to(self.device)
        self.target_critic2 = QNetwork(env.observation_space.shape[0], action_dim, config.get("hidden_dim", 256)).to(self.device)
    ...
    def update(self, batch_size=256):
        ...
        state = torch.FloatTensor(state).to(self.device)
        action = torch.LongTensor(action).to(self.device) if self.is_discrete else torch.FloatTensor(action).to(self.device)
        ...
        with torch.no_grad():
            ...
            if self.is_discrete:
                next_action_one_hot = F.one_hot(next_action, num_classes=self.actor.action_dim).float()
            ...
            target_q = reward.unsqueeze(1) + (1 - done.unsqueeze(1)) * self.config.get("gamma", 0.99) * (target_q - self.config.get("alpha", 0.2) * next_log_prob.unsqueeze(1))
        
        if self.is_discrete:
            action_one_hot = F.one_hot(action, num_classes=self.actor.action_dim).float()
        ...
        if self.is_discrete:
            new_action_one_hot = F.one_hot(new_action, num_classes=self.actor.action_dim).float()
        ...
        for param, target_param in zip(self.critic1.parameters(), self.target_critic1.parameters()):
            target_param.data.copy_(self.config.get("tau", 0.005) * param.data + (1 - self.config.get("tau", 0.005)) * target_param.data)
        for param, target_param in zip(self.critic2.parameters(), self.target_critic2.parameters()):
            target_param.data.copy_(self.config.get("tau", 0.005) * param.data + (1 - self.config.get("tau", 0.005)) * target_param.data)
        
        self.episode_losses.append(actor_loss.item() + critic1_loss.item() + critic2_loss.item())
